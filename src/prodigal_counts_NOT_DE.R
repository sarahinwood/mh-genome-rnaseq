...
##list of DE prodigal genes
lrt_degs <- fread("output/deseq2/tissue_LRT/sig_degs.csv")
stage_degs <- fread("output/deseq2/stage_WT/sig_degs.csv")
lrt_prodigal_DEGs <- lrt_degs %>% filter(str_detect(lrt_degs$rn, 'MHPROD'))
stage_prodigal_DEGs <- stage_degs %>% filter(str_detect(stage_degs$rn, 'MHPROD'))
prodigal_degs <- full_join(lrt_prodigal_DEGs, stage_prodigal_DEGs)
...
prodigal_counts <- counts_table %>% filter(str_detect(counts_table$rn, 'MHPROD'))
fwrite(data.table(prodigal_counts, keep.rownames=TRUE), "output/deseq2/prodigal_genes/prodigal_counts.csv")
##not DE prodigal genes
annot_counts <- filter(prodigal_counts, !(rn %in% prodigal_degs$rn))
